...
export async function loadCatalog(): Promise<AreaCatalog> {
  const response = await fetch('/catalog.json', { signal: AbortSignal.timeout(15000) })
  ...
  if (
    ...
    !catalog.areas.some((area) => area.id === catalog.defaultArea)
  ) {
...
export async function loadScenario(scenarioRoot: string): Promise<ScenarioData> {
  ...
  if (
    !metadata.grid ||
    !Number.isInteger(metadata.grid.width) ||
    !Number.isInteger(metadata.grid.height) ||
    metadata.grid.width < 2 ||
    metadata.grid.height < 2 ||
    !(metadata.grid.cellSizeMetres > 0) ||
    !metadata.timeline ||
    !Number.isInteger(metadata.timeline.frameCount) ||
    metadata.timeline.frameCount < 1 ||
    !(metadata.timeline.depthScaleMetres > 0) ||
    !(metadata.timeline.intervalSeconds > 0) ||
    !Array.isArray(metadata.members) ||
    metadata.members.length !== 3 ||
    new Set(metadata.members.map((member) => member.id)).size !== 3 ||
    metadata.members.some((member) => !['copernicus', 'fabdem', 'srtm'].includes(member.id))
  ) {
    throw new Error('Scenario metadata is incomplete or unsupported')
  }
  ...
  const members: MemberGrid[] = metadata.members.map((member, index) => {
    ...
    if (
      terrain.length !== cellCount ||
      depth.length !== cellCount ||
  ...
  for (let index = 0; index < cellCount; index += 1) {
    ...
    medianDepth[index] =
      values.reduce((sum, value) => sum + value, 0) - Math.min(...values) - Math.max(...values)
  }
  let roadImpact
  if (metadata.roadImpact) {
    ...
    const [timelineDepth, timelineAgreement, peakDepth, peakAgreement, lengths] = await Promise.all(
      [
        fetchBuffer(`${scenarioRoot}/${impact.timelineDepthFile}`).then(
          (value) => new Uint16Array(value),
        ),
        fetchBuffer(`${scenarioRoot}/${impact.timelineAgreementFile}`).then(
          (value) => new Uint8Array(value),
        ),
        fetchBuffer(`${scenarioRoot}/${impact.peakDepthFile}`).then(
          (value) => new Uint16Array(value),
        ),
        fetchBuffer(`${scenarioRoot}/${impact.peakAgreementFile}`).then(
          (value) => new Uint8Array(value),
        ),
        fetchBuffer(`${scenarioRoot}/${impact.lengthFile}`).then(
          (value) => new Float32Array(value),
        ),
      ],
    )
    if (
      timelineDepth.length !== impact.frameCount * impact.lineCount ||
      timelineAgreement.length !== impact.frameCount * impact.lineCount ||
      peakDepth.length !== impact.lineCount ||
      peakAgreement.length !== impact.lineCount ||
...
export function timelineDepthForView(
  ...
  for (let index = 0; index < cellCount; index += 1) {
    ...
    result[index] =
      (values[0] + values[1] + values[2] - Math.min(...values) - Math.max(...values)) *
      depthScaleMetres
  }
  return result
}
...
export async function loadUrbanContext(contextRoot: string): Promise<UrbanContextData> {
  ...
  const [
    ...
    fetchBuffer(`${contextRoot}/${metadata.buildings.coordinateFile}`).then(
      (value) => new Float32Array(value),
    ),
    fetchBuffer(`${contextRoot}/${metadata.buildings.indexFile}`).then(
      (value) => new Uint32Array(value),
    ),
    fetchBuffer(`${contextRoot}/${metadata.buildings.heightFile}`).then(
      (value) => new Float32Array(value),
    ),
    fetchBuffer(`${contextRoot}/${metadata.buildings.heightSourceFile}`).then(
      (value) => new Uint8Array(value),
    ),
    fetchBuffer(`${contextRoot}/${metadata.buildings.sourceFile}`).then(
      (value) => new Uint8Array(value),
    ),
    fetchBuffer(`${contextRoot}/${metadata.network.coordinateFile}`).then(
      (value) => new Float32Array(value),
    ),
    fetchBuffer(`${contextRoot}/${metadata.network.indexFile}`).then(
      (value) => new Uint32Array(value),
    ),
  ])
  if (
    buildingIndex.length !== metadata.buildings.count * 2 ||
