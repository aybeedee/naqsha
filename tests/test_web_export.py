...
def test_uint16_web_asset_is_little_endian_row_major(tmp_path: Path):
    path = tmp_path / "timeline.u16"
    write_uint16(path, np.array([[1, 256], [512, 1024]]))
    decoded = np.frombuffer(path.read_bytes(), dtype="<u2")
    assert decoded.tolist() == [1, 256, 512, 1024]
